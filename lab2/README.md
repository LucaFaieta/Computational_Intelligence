...
## Solution Overview
The key concept of this solution is to divide game positions in two categories, based on the advantage that they give to the players. Any move from a player takes the state of a game from a position to one of the next possible ones. N-positions are positions that give advantage to the current player (i.e. the one who just made the move), while P-positions are those who give advantaged to the opponent. N and P positions are determined by a function called nim-sum.
...
Our approach consists of generating a certain number of possible expression for a given Nim board, and then to recombine them following the Genetic Programming paradigms of recombination and mutation until a winning solution is found.
...
In this case, there are three theorems that run the winning strategy for the Nim game
(Berlekamp 1988):
...
The first thing is to compute the state of every node (P-position or N-position) for a certain expression, and then we traverse the tree to analyze its properties with respect to the rules above.

At this point, given a chromosome, the `fitness_function()`  returns an integer, evaluated as the sum of the number of violations that it encountered while traversing the tree
...
The algorithm then executes the following steps, until an optimal solution is found or a number of iterations is reached:
  ...
  3. **survival selection**: the fittest offspring is selected, and added to the dna if its score is better than the lowest score in the list

## Our results
We tested this strategy against the ones proposed in class and ran the match 50 times for each configuration. All simulations are ran over a three-column Nim Map
...
All parameters for the evolutionary algorithm are at the start of the file
